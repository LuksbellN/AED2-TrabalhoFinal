...
#define TAM_INCREMENTO                                                         \
  1000 // Quantidade de incremento quando o vetor precisar crescer
#define MAX_LINE_LENGTH                                                        \
  500 // Quantidade máxima de caracteres por linha do arquivo
...
int ler_arquivo_vetor(const char *caminho, struct candidato **vetor,
  ...
  while (fgets(linha, sizeof(linha), arquivo)) {
    ...
    while (token != NULL && i < 10) {
      switch (i) {
      case 0:
        strcpy((*vetor)[*indice].estado, token);
        break;
      case 1:
        strcpy((*vetor)[*indice].cidade, token);
        break;
      case 2:
        (*vetor)[*indice].nr_candidato = atoi(token);
        break;
      case 3:
        strcpy((*vetor)[*indice].cargo, token);
        break;
      case 4:
        strcpy((*vetor)[*indice].nm_candidato, token);
        break;
      case 5:
        strcpy((*vetor)[*indice].nm_urna_candidato, token);
        break;
      case 6:
        strcpy((*vetor)[*indice].sg_partido, token);
        break;
      case 7:
        strcpy((*vetor)[*indice].genero, token);
        break;
      case 8:
        strcpy((*vetor)[*indice].grau_instrucao, token);
        break;
      case 9:
        strcpy((*vetor)[*indice].cor_raca, token);
        break;
      }
      token = strtok(NULL, ";");
      i++;
...
void exibir_dados(struct candidato *vetor, int n) {
  for (int i = 0; i < n; i++) {
    printf("Estado: %s, Cidade: %s, Nr: %d, Nome: %s\n", vetor[i].estado,
           vetor[i].cidade, vetor[i].nr_candidato, vetor[i].nm_candidato);
  }
}