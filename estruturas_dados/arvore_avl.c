...
NoCandidatoAVL *insere_arvore_binaria_AVL(NoCandidatoAVL *raiz, const struct candidato *novo_candidato)
{
    ...
    if (balanco > 1 && comparar_ordenacao_arvore(*novo_candidato, raiz->esquerda->candidato) < 0)
        return rotacionar_direita(raiz);

    // Caso Direita-Direita
    if (balanco < -1 && comparar_ordenacao_arvore(*novo_candidato, raiz->direita->candidato) > 0)
...
NoCandidatoAVL *rotacionar_direita(NoCandidatoAVL *raiz)
{
    ...
    raiz->altura = 1 + (altura(raiz->esquerda) > altura(raiz->direita) ? altura(raiz->esquerda) : altura(raiz->direita));
    p1->altura = 1 + (altura(p1->esquerda) > altura(p1->direita) ? altura(p1->esquerda) : altura(p1->direita));
...
NoCandidatoAVL *ler_arquivo_arvore_binaria_AVL(const char *caminho)
{
    ...
    while (fgets(linha, sizeof(linha), arquivo))
    {
        ...
        arvore = insere_arvore_binaria_AVL(arvore, &novo_candidato);
    }
